...
//--------------------------------------------------------------------------------
  ...
  std::set<face> SimplicialComplex::myifaces(const long& i0, const long& i1 )
  {// so far I try to avoid using this function. Namely for IamKDecomposable
  ...
  bool SimplicialComplex::IamSheddingFace(const face& f) const // iff no facet of fdel_(star_SC(f))(f)) is a facet of fdel_SC(f) [Wo] Remark 3.12
  {
    ...
    fdel_star=fdel_star.myFaceDelSC(f);      //i sit possible to do directly SimplicialComplex fdel_star=myStarSC(f).myFaceDelSC(f) ?
