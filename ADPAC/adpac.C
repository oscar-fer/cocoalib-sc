...
void program()
{
 ...
 //ideal JJ=ADPAC(6,3);
 SimplicialComplex Delta=ADPAC(6,3);
 // cout << Delta.myNumFacets() << endl;
 // cout << ADPAC(6,3) << endl;
 // std::set<facet> LiF=Delta.myifaces(0,2);
 // // cout << LiF.size() << endl;
  cout << Delta << endl;
 std::set<face> L=Delta.myifaces(1,1);
 for (std::set<face>::const_iterator it=L.begin(); it!=L.end(); ++it)
   if (Delta.IamSheddingFace(*it)) cout <<*it << endl;  
